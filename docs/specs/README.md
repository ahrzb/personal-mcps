...
| 9 | [YAML config, diff, apply](admin-and-config/09-yaml-config.md) | The declarative file: its exact shape for apps, roles, redaction, and grants, and the desired-state semantics of `diff`/`apply` — deletes by absence, warn-vs-error per app kind, `pmcp` excluded, destructive flags. |
