...
| 3 | [Architecture](overview/03-architecture.md) | The request picture: the Worker as the single trust boundary, the `AppConnection` DO owning the hibernatable socket and the cached catalog, proxied upstreams dialed directly with no DO, and the rule that apps always dial *in*. |
...
| 5 | [Data model](data-model/05-data-model.md) | The D1 schema of record: `app`, `agent`, `grant_`, `approval`, `token`, `audit`, `push_subscription`, `upstream_oauth_state` — column by column, with the constraints and the comments that carry the rules. Also what better-auth owns and what the DO keeps in its own SQLite. |
| 6 | [Reverse connection protocol](tunnel/06-reverse-connection-protocol.md) | The tunnel wire: the upgrade status matrix (401 fatal / 403 archived) and close codes, `hub/register` with role validation and role-drift auditing, `hub/replaced` and newest-wins, the 10 s registration deadline, ping-based liveness, the 30 s correlation timeout — plus the app lifecycle (provisioned → online/offline ↔ archived → deleted). |
| 7 | [Consumer-facing proxy](gateway/07-consumer-facing-proxy.md) | The door, and the longest section: aggregated vs scoped endpoints, authentication and the 401/404 anti-enumeration matrix, filter resolution and pattern semantics, the fixed check order (`-32001` → `-32002` → `-32003` → `-32000`), the approval flow with its CAS claim and MRTR handling, caller identity forwarding and `hub/*` strip-then-set, upstream OAuth for proxied apps, and sensitive-field redaction. |
| 8 | [Admin MCP](admin-and-config/08-admin-mcp.md) | The built-in `pmcp` app: every admin tool and its shape, the uniform `pmcp`-slug rejection, the reserved-and-virtual slug, the parity invariant (anything the UI or CLI can do has a tool) with its pinned exceptions, and the `GET /api/whoami` contract the CLI depends on. |
| 9 | [YAML config, diff, apply](admin-and-config/09-yaml-config.md) | **Superseded by §22** — the provider replaces this surface and `pmcp diff`/`apply` are removed. Retained until that lands: the declarative file's exact shape for apps, roles, redaction, and grants, and the desired-state semantics of `diff`/`apply` — deletes by absence, warn-vs-error per app kind, `pmcp` excluded, destructive flags. |
...
| 22 | [The OpenTofu provider](provider/22-opentofu-provider.md) | `terraform-provider-pmcp`, living in its own repo and managing hub contents while Wrangler owns the Worker: the `pmcp_adm_` admin credential and its own table, write-only upstream headers with a convergent applied-version, the four resources and two data sources, the behavioural parity oracle that gates this repo's CI, the retirement of §9's planner, both flakes and the terranix module, and the acceptance rig. |
