...
| **web pages** | ~~Two renderings, one design language~~ One rendering (§13): `/apps/*`, `/agents/*` and `/audit` *(2026-09-21, decision 36)* are a React SPA (TanStack Query + Router, shadcn on Base UI) over the cookie-authenticated JSON surface at `/api/hub`; login/device, seven-pane Settings including Execution, approvals and the OAuth consent screen ~~stay server-rendered (Hono JSX)~~ *(2026-09-23, decision 38: are too, first holding their look constant, then on shadcn)*. ~~Both front~~ Every page fronts shared operations except the pinned browser/auth/export exceptions. |
