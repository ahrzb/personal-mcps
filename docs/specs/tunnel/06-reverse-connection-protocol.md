## 6. Reverse connection protocol (tunneled app ↔ hub)

This section is tunnel-kind only; proxied apps have no connection of their own.

Transport: WebSocket to `wss://<host>/connect`, `Authorization: Bearer pmcp_app_…`.
The Worker verifies the app token — checking the token row's `kind` column
explicitly, not just the prefix — resolves the app (and its owner), and hands the
socket to `AppConnection` DO `getByName(app.id)` (the opaque id, not
`user/slug`, so deleting a user and recreating the same username can never rebind to a
stale DO), which calls `ctx.acceptWebSocket(ws, [app.id])`.

Upgrade response matrix — the status codes carry meaning for the client library:
- **401** — no/invalid/expired/revoked token, wrong token kind (`pmcp_agt_`, session), or
  a token whose app row is gone or is `kind: proxy`. Client treats this as fatal:
  stop and surface a credentials error (no retry loop on a dead credential).
- **403** — means exactly one thing: the app is archived. Client keeps retrying at
  max backoff so unarchiving heals automatically.
- Close `4001` after establishment (token revoked / app deleted) is treated like
...
1. **Control** — JSON-RPC methods prefixed `hub/`, handled by the client library, never
   ...
   - `hub/register` (client → hub, first message):
     ...
     *(amended 2026-08-26, §20: a role's value is **either** that bare pattern list —
     ...
     per-family dimension is the load-bearing half of this rule now. An app that
     re-registers `"reader": ["get_news"]` as `"reader": {"tools": ["get_news"],
     "resources": ["file:///*"]}` has an unchanged **tools** set and has just handed every
     granted agent the whole resource keyspace; a family-blind comparison sees a subset
     and writes nothing. Since §2's trust-boundary paragraph makes this row the only
     visibility into an app widening itself, comparing per family is what keeps that
     ...
     tools-only app log a spurious catalog-warm failure for three families it never
     ...
     app already in the field runs a library that has never heard of it. The
     ...
     **tools only**, exactly as it does today. An app whose library predates this
     ...
     the app is then reachable only by admin tokens or agents granted the
     built-in `all` role.
   - `hub/replaced` (hub → client, notification): a newer connection for the same slug
     ...
     **acceptance** of the new socket (`ctx.acceptWebSocket`), before its `hub/register`
     is seen — the DO never holds two **app** sockets, preserving §2's
...
2. **MCP** — everything else. The hub acts as the MCP *client*; the app is the MCP
   *server*. v1 forwarded `tools/list` and `tools/call`; *(amended 2026-08-26, §20:
   ...
   These are the only app-originated frames the DO
...
that sent none gets `{}`, which per MRTR rules means the app must not emit
...
unregistered socket — until `hub/register` succeeds the app is offline and
...
1. **Provisioned** — the owner creates the row (`app_create` / `apply`) and mints an
   ...
   app has no roles and lists no tools.
2. **Online / offline** — runtime status: **online** means the DO holds a live,
   **registered** socket (surfaced in `app_list` / `pmcp ls`); a socket accepted
