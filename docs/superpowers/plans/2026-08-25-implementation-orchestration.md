...
- 2026-09-03 — **Roadmap step 6 landed (inline, own deploy) — G5, G11, G50, G51 closed.**
  ...
  agents.
- 2026-09-03 — **Roadmap step 7 landed (inline, own deploy — not riding step 8 as the plan
  said; every step ships alone now) — G6 closed.** `f5ab0fe` (rows) + `8d9ccfb` (`feat:`),
  plan `2026-09-03-step7-pwa-icons.md`. No agents, bodies first. `pages/icon.ts` carries
  `ICON_192` / `ICON_512` as base64 (typed `Uint8Array<ArrayBuffer>` — the worker's
  `BodyInit` refuses `ArrayBufferLike`), generated by `scripts/icon.mjs` through a scratch
  writer so the 9 K characters never passed through a hand; two routes under `PNG`, no
  cache headers; `paths.icon192` / `icon512`; `ROUTES` + `MOUNTS` rows (comment counts
  eleven browser / five machine); the manifest's `icons` exactly the 192/512 pair with
  `type`, no `sizes: "any"`, no `purpose`; `layout.tsx` links `rel="icon"` and
  `rel="apple-touch-icon"` at the 192 (`ICON` beside the other three spellings); §2's
  reserved list amended to five shell assets. Rows read the PNG signature and IHDR width
  off each manifest src's bytes, pin the declaration, and the head's two links against
  /login's none. One test-side correction: hono renders `<link … />`, so the head match
  stops at the `href`. Gate: 45 / 1453 / 0 — the inventory's todo count is now zero —
