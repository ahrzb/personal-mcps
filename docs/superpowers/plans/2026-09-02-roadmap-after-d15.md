...
| 14 | Push encoding swap (G23) — the library swap the owner chose | — | inline fix | 3a | **Landed 2026-09-12.** `aes128gcm` / `vapid t=,k=` on the wire; the harness receiver opens RFC 8291 §5's own example bytes; Apple's real endpoint reaches `BadWebPushToken` (only the fake device token is wrong) where the old dialect got `403 BadVapidPublicKey` |
...
| G23 | ~~Push bodies are draft-04 `aesgcm` + `Authorization: WebPush`, not §13's RFC 8291/8292 — Apple Web Push refuses every notification~~ **answered 2026-09-12: library swap**, landed as step 14 | `push-service.ts` pins the `aes128gcm` label so `approvals.test.ts` cannot pass under the old encoding; §13:515-521 carries the amendment |
...
| G23 | **closed 2026-09-12** | dispatch | Web Push | was draft-04 `aesgcm` + `WebPush` auth; now RFC 8291/8292 on `@block65/webcrypto-web-push`, verified against Apple's live endpoint | push.ts:1-38; §13:515-521; step 14 |
