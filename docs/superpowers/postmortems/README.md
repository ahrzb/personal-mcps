...
- [2026-09-23 — the post-deploy smoke failed hub_execute because the rollout severed its tunnel](2026-09-23-smoke-tunnel-severed-by-rollout.md) — the inner echo found the tunnel offline 16 s after the deploy and the client reconnected a second later; not a QuickJS cold start. The probe's -32000 row named no cause, which is now fixed
