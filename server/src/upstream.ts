// upstream.ts — the proxied-app backend and the sole custodian of upstream credentials.
//
// OWNS the whole "hub as MCP client" side of proxied apps: per-request dialing of the
...
// upstream credential — `app.upstream_auth_json` is always an AES-GCM envelope under
...
// are never echoed to a consumer. Registry owns the `app` rows' lifecycle; this
 ...
 * `app_get` and the `/apps` page. Total over both auth modes:
 * `not_connected` — no credential envelope is stored (fresh app, after
 ...
 * apps report `connected` iff headers are stored and can never be `needs_reconnect`.
 */
 ...
 * gateway's JSON-RPC mapping. `message` is errors.unavailable's — the generic "app
...
/**
 * The proxied `AppBackend` — the pipeline's dispatch target for `kind: "proxy"`.
 * Stateless per request: no connection, no catalog cache; every method loads the
 * app's stored credentials and dials the upstream fresh.
 */
export const upstreamBackend: AppBackend = {
  /**
  ...
  async listTools(app, ctx) {
    // deps: D1 `app` · crypto.subtle (AES-GCM envelope) · @modelcontextprotocol/client Client (Streamable HTTP) · audit.record (refresh failure)
    return liveList<Tool>(app, "tools/list", "tools");
  },

  /**
   * §20.2's three further listings, live and uncached like `listTools` above — §20.5 pins
   * "proxied apps cache nothing at all" for every family, not tools alone. A failing
  ...
  async listPrompts(app, ctx) {
    return liveList<Prompt>(app, "prompts/list", "prompts");
  },
  async listResources(app, ctx) {
    return liveList<Resource>(app, "resources/list", "resources");
  },
  async listResourceTemplates(app, ctx) {
    return liveList<ResourceTemplate>(app, "resources/templates/list", "resourceTemplates");
  },
   ...
   * oauth access token is refreshed before dialing; a failed refresh flips the app to
   * needs_reconnect, writes `upstream.oauth_refresh_failed`, and throws. Emits
   * `X-Pmcp-Principal` / `X-Pmcp-Roles` (from ctx) only when the app opted into
...
/**
 * One JSON-RPC round trip to the app's upstream endpoint, with every failure class
 ...
 * app opted into `forward_identity` (§7 — default off, and consumer headers are
...
async function dial(
  ...
  const row = await proxyRowOf(app.id);
  ...
  const headers: Record<string, string> = {
    ...
    ...(await credentialHeaders(app, row)),
  };
 ...
 */
async function liveList<T>(app: App, method: string, key: string): Promise<T[]> {
  const relayed = await dial(app, { jsonrpc: "2.0", id: crypto.randomUUID(), method, params: {} });
...
/**
 * The ONE read of an app's proxy-auth columns — exactly the set ProxyRow names, so the
 * type and the statement cannot describe different rows and a column added there is one
 * edit here. Null for an app id that is gone; a TUNNELED app answers a row of nulls,
 * which every caller reads as "no upstream" through the field it already checks.
 */
async function proxyRowOf(appId: string): Promise<ProxyRow | null> {
  // deps: D1 `app`
...
/**
 * What the stored bytes MEAN for this app's auth mode — computed once, from a ProxyRow,
 ...
 * absence of a credential: a fresh app, what Disconnect left, or an envelope this mode
 * cannot use at all.
 */
...
async function credentialOf(row: ProxyRow): Promise<Credential> {
  ...
  if (row.upstream_auth_mode === "oauth") {
    ...
    // /apps renders as Connect rather than Reconnect. Everything past here is about what
...
/**
 * The credential half of one dial: the headers an app's stored envelope contributes,
 ...
 */
async function credentialHeaders(app: App, row: ProxyRow): Promise<Record<string, string>> {
  ...
  const live = await spendable(app, credential, row.upstream_auth_json);
...
async function spendable(
  ...
  return refresh(app, bundle, sealed);
}
 ...
 * only in memory is an app that works exactly once.
 *
 * ponytail: nothing SERIALIZES refresh, so two concurrent calls on one stale app both
 ...
 * credential. A per-app lock (the tunnel DO, or D1 `BEGIN IMMEDIATE`) is the upgrade if
...
async function refresh(
  ...
  if (!(await store(app.id, { ...bundle, needsReconnect: true }, sealed))) {
    const winner = await storedBundle(app.id);
    if (winner !== null) return winner;
  }
  await record(env.DB, {
    ownerId: app.ownerId,
    principal: await ownerPrincipal(app.ownerId),
...
/** The oauth bundle stored for an app right now, if it is one this hub may spend — the
 *  re-read behind refresh's compare-and-set, and the only place a verdict is re-taken. */
async function storedBundle(appId: string): Promise<OAuthBundle | null> {
  const row = await proxyRowOf(appId);
 ...
 * is the AS's own no and the ONLY thing that may flip an app to needs_reconnect; the
...
/**
 * Seal one bundle into the app's credential column — the single write path. `expected`
 ...
 */
async function store(appId: string, bundle: Bundle, expected?: string | null): Promise<boolean> {
  ...
  const statement =
    expected === undefined
      ...
      : (env.DB as D1Like)
          .prepare(
            `UPDATE app SET upstream_auth_json = ?
              WHERE id = ? AND upstream_auth_json IS ?`,
          )
          .bind(sealed, appId, expected);
  const { meta } = await statement.run();
  return meta.changes === 1;
}

/**
 * Starts the interactive OAuth connect flow for an `auth: oauth` proxied app and
 ...
 * records a single-use state row bound to {owner, app, expected issuer + token
 * endpoint, PKCE verifier} and to `session` — expiring 10 minutes out. Nothing is stored
 * on the app row yet. Rejects (`HubError`) an app that is headers-mode or not
 * proxied: Connect is the oauth mode's one credential path.
 */
export async function beginConnect(app: App, session: ConnectSession): Promise<URL> {
  // deps: fetch (RFC 9728 + AS metadata discovery, DCR) · crypto (state nonce, PKCE verifier) · D1 `upstream_oauth_state` · D1 `app`
  const row = app.kind !== "proxy" ? null : await proxyRowOf(app.id);
  // Each mode has exactly one credential path (§8), and this is the oauth mode's: a
  // tunneled app has no upstream at all, a headers one has setHeaders.
  if (row?.upstream_auth_mode !== "oauth" || !row.upstream_url) {
    throw new HubError(INVALID_PARAMS, "this app does not connect through OAuth");
  }
  ...
  await (env.DB as D1Like)
    .prepare(
      `INSERT INTO upstream_oauth_state
         (state, owner_id, app_id, session_id, issuer, token_endpoint, client_id,
...
/**
 * Wipes the stored credential envelope (either auth mode), leaving the app
 ...
 * field are untouched. Idempotent: disconnecting an already-empty app is a no-op.
 * An auth-mode flip never routes through here — clearing the envelope on a flip is
 * registry.updateApp's row invariant, and its audit row is the admin op's.
 */
export async function disconnect(app: App): Promise<void> {
  // deps: D1 `app` · audit.record
  await (env.DB as D1Like)
    .prepare(`UPDATE app SET upstream_auth_json = NULL WHERE id = ?`)
    .bind(app.id)
    .run();
 ...
 * credential path.
 */
export async function setHeaders(app: App, headers: Record<string, string>): Promise<void> {
  // deps: crypto.subtle (AES-GCM envelope) · D1 `app` · errors.HubError
  // Each mode has exactly one credential path (§8), and this is the headers mode's: a
  // tunneled app has no upstream at all (and no auth mode), an oauth one has Connect.
  const row = app.kind !== "proxy" ? null : await proxyRowOf(app.id);
  if (row?.upstream_auth_mode !== "headers") {
    throw new HubError(INVALID_PARAMS, "this app does not store upstream headers");
  }
  await store(app.id, { kind: "headers", headers });
}
 ...
 * app's mode is no credential at all.
 */
 ...
 * app says no more about it than an export of a live one.
 */
type OAuthBundle = {
  kind: "oauth";
  accessToken: string;
 ...
 * seams take a `App` rather than a principal — so the one name the ledger needs is
 * looked up from the namespace the app already carries.
 */
...
/**
 * Reports the app's upstream credential state (see `UpstreamConnectionStatus` for
 ...
 * behind. Backs `app_list` / `app_get` and the `/apps`
 * Connect/Reconnect/Disconnect buttons.
 */
export async function connectionStatus(app: App): Promise<UpstreamConnectionStatus> {
  ...
  // app rather than about this function (see Credential).
  const credential = await credentialOf(row);
  return "unusable" in credential ? credential.unusable : "connected";
}

/**
 * §7's availability-first verdict for a PROXIED app, as the refusal itself rather than
 ...
 * an upstream failure at all — the app was never connected — so it refuses with the
 * gateway's own class-free -32000, which is what keeps a never-configured app from
 * being reported as a broken credential.
 */
export async function availability(app: App): Promise<HubError | null> {
  // deps: connectionStatus
  const status = await connectionStatus(app);
