 ...
 * call either does or does not arrive at a fake app on a live WebSocket. That fake
 * app's INVOCATION COUNTER is the oracle this whole file turns on — "exactly once"
 ...
 * dispatch; availability-first in BOTH directions (a known-unavailable app is
 ...
 * cannot map its own grant patterns); and the healing arc, where an app with a COLD
 ...
 * a real AppConnection, and interleavings that must not race a sibling worker
...
import type { FakeApp } from "../harness/fake-app";
 ...
 * call — past approvals.check, past the atomic claim, frame arrives at the app,
 * app answers — and are exactly where a SELECT-then-dispatch implementation differs
 * from a CAS one. The two unscoped steps are the world changing underneath: the owner
 * deciding out of band, and the app going away or coming back (the
 ...
 *   has settled or a frame has reached the app, which is as close to "past the claim"
 ...
 *   the way FakeApp gives hang/release; until then a row must not encode an ordering
 *   whose only expression is the order of its `claimed` steps.
 * · `dispatched` and `answered` are real: both are observed at the fake app, which is
 ...
 * app's counter afterwards — the exactly-once oracle. `outcomes` is per leg, so a
 ...
 * "never touched" (an unavailable app).
 *
 ...
 * conditions with one input changed — the app online rather than offline, the tool
...
export const casInterleavings: readonly CasInterleavingRow[] = [
  // The fixture these rows are written against, named once: one connected tunneled app
  // `notes` serving `search`, one agent holding `reader` in APPROVAL mode, and
  ...
  //   restore: this table has no column for the app's ANSWER kind, so
  //   `input_required`-restores-the-row lives in cases 16-19, where the fake app's
  ...
  {
    name: "§7 step 1 · one leg on one approved pass: check, claim, dispatch, answer — the pass is consumed and the app is invoked exactly once",
    legs: ["A"],
    ...
    outcomes: { A: "executed" },
    finalStatus: "used",
    twin: "§7 step 1 · one leg on one approved pass: check, claim, dispatch, answer — the pass is consumed and the app is invoked exactly once",
  },
  ...
  // §7's availability-first rule: an app the hub already knows cannot execute "fails
  ...
  {
    name: "§7 · a leg arriving at an already-offline app is refused -32000 and the approved pass is left untouched — the owner never re-approves for a reconnect",
    legs: ["A"],
    startsWith: "approved_pass",
    schedule: [{ at: "app_offline" }, { leg: "A", at: "checked" }],
    invocations: 0,
    outcomes: { A: -32000 },
  ...
  // "an existing approved pass survives untouched; the agent's retry once the app
  ...
  // row that releases `app_online`, and the interleaving case 13 names.
  {
    name: "§7 · the pass that survived an offline refusal is spent by the retry once the app returns: -32000, then the identical leg executes exactly once on that same untouched row",
    legs: ["A", "B"],
    startsWith: "approved_pass",
    schedule: [
      { at: "app_offline" },
    ...
    finalStatus: "used",
    twin: "§7 · the pass that survived an offline refusal is spent by the retry once the app returns: -32000, then the identical leg executes exactly once on that same untouched row",
  },
  ...
  {
    name: "§7 step 1 · an app that goes away between check and claim: -32000 with the pass unconsumed, so availability is re-read after the lookup and not only before it",
    legs: ["A"],
    startsWith: "approved_pass",
    schedule: [
      { leg: "A", at: "checked" },
  ...
  // app (every `tools/call` is at-most-once, §15), so reverting the row would risk a
  ...
  // numbers disagree.
  {
    name: "§7 step 1 · a socket dropping after the claim and after the frame arrived: the consumer gets -32000, the app counted the call, and the pass stays used",
    legs: ["A"],
    startsWith: "approved_pass",
    schedule: [
      { leg: "A", at: "checked" },
 ...
 * fake app and the gate seam expose. Reads the counter and the row afterwards.
...
export async function runCasInterleaving(
  ...
  // deps: harness/seed · harness/fake-app · cloudflare:workers exports.default.fetch · src/approvals.Approvals
   ...
   *  before the claim) or its frame reached the app (its claim won). */
    ...
    switch (step.at) {
      ...
      case "dispatched":
        ...
        expect(
          ...
          `"${row.name}": leg ${step.leg}'s frame never reached the app`,
        ).toBe(true);
        break;
      case "answered":
 ...
 *  `search`, one agent holding `reader` in APPROVAL mode. */
...
/** Planted values: one masked by the tool's schema, one by the app's `redact` config —
 ...
 *  and `profile.email` is what the app's config `redact` covers. */
...
/** What the app answers a complete call with. */
const ANSWER = {
...
async function seedUnconnected(): Promise<Fixture> {
  const ns = await seedNamespace(env.DB, {
    ...
    agents: [
      {
        slug: AGENT,
        grants: { [APP_SLUG]: [{ role: "reader", mode: "approval" }] },
        tokens: [{ as: AGENT }],
      },
    ],
  });
  ...
  return {
    ...
    sockets: [],
    fake: undefined as unknown as FakeApp,
  };
}

/** Dial (or re-dial) the app's socket and wait until its catalog is cached. */
async function goOnline(fixture: Fixture): Promise<void> {
  await dial(fixture, [SEARCH_TOOL]);
  await untilStatus(fixture.app.id, "online");
  const cached = await untilCataloged(await appRow(fixture));
...
async function dial(fixture: Fixture, tools: Tool[]): Promise<FakeApp> {
  const app = await connectFakeApp({
    origin: ORIGIN,
    token: fixture.ns.tokens.app.token,
  ...
  opened.push(app);
  fixture.sockets.push(app);
  fixture.fake = app;
  return app;
}

/** Close the current socket and wait until the hub reports the app offline. */
...
/** One gated `tools/call` as the agent makes it — the shape every leg sends. */
...
function agentCall(
  ...
  return rpc(fixture, fixture.ns.tokens[AGENT].token, APP_SLUG, callMessage(args, extraParams, tool));
}

/** One leg of an interleaving: the identical call, fired and not awaited. */
function enterLeg(fixture: Fixture): Promise<Answer> {
  return agentCall(fixture);
}
 ...
 * app under the owner's own session, which is the path the web page and the CLI both
...
async function seedApprovedPass(fixture: Fixture): Promise<string> {
  const opened = await agentCall(fixture);
...
/** How many audit rows the namespace holds for one event — optionally on one app, which
 *  is how a call on the tunneled app is told from the owner's own admin call. */
async function auditRows(fixture: Fixture, event: string, app?: string): Promise<number> {
  return (await query(env.DB, fixture.ns.owner.userId, { event, app })).total;
}
...
describe("§7 the approval loop over a real tunnel", () => {
  it("1. §7 step 2 · a gated call answers -32003 carrying approvalId, approvalUrl and expiresAt, and the app's invocation counter stays at zero", async () => {
    const fixture = await seedFixture();

    const gated = await agentCall(fixture);
    ...
    expect(servedCalls(fixture), "a gated call never reaches the app").toBe(0);
  });
  ...
  it("2. §7 step 2 · a retry while still pending returns the SAME approvalId, with no second row, no second approval.requested audit row, and no second push", async () => {
    ...
    const first = await agentCall(fixture);
    const retry = await agentCall(fixture);
  ...
  it("3. §7 step 4 · after approve, the identical retry executes exactly once — counter 1 (the allow-twin every refusal in this file is measured against)", async () => {
    ...
    // Two rows on the app — the refusal and the run — beside the owner's own decision,
    // which is a `tools/call` on the builtin like every other admin op (§8).
    expect(await auditRows(fixture, "tools/call", APP_SLUG)).toBe(2);
  ...
  it("4. §7 step 4 · a second identical call after consumption opens a fresh pending and the counter stays at 1", async () => {
    ...
    expect((await agentCall(fixture)).body.result, "the pass was not spent").toBeDefined();

    const again = await agentCall(fixture);
  ...
  it("5. §7 · arguments differing in a visible field do not match the pass: fresh -32003, counter unchanged", async () => {
    ...
    const different = await agentCall(fixture, { ...ARGS, q: "a visibly different question" });
  ...
  it("6. §7 step 4 · a rejected request answers -32003 with a fresh pending row, and the rejected row stays rejected", async () => {
    const fixture = await seedFixture();
    const rejected = approvalIdOf(await agentCall(fixture));
  ...
  it("7. §7 · a pass past expires_at reads as expired everywhere and emits approval.expired exactly once — driven through the injected clock against limits.APPROVAL_WINDOW_MS, never slept", async () => {
    const fixture = await seedFixture();
    const pending = approvalIdOf(await agentCall(fixture));
  ...
  it("8. §7 · owners are never approval-gated: the same tool called by the owner executes with no approval row created at all", async () => {
    ...
    const answer = await rpc(fixture, session.token, APP_SLUG, callMessage());
...
describe("§7 availability-first, both directions", () => {
  it("12. §7 · a known-offline tunnel is refused -32000 BEFORE the gate reads anything: no pending row, no audit approval.requested, no push — twin: the same call against an online app opens the pending", async () => {
    const fixture = await seedFixture();
    await goOffline(fixture);

    const refused = await agentCall(fixture);
    ...
    // The twin, one state different: the same call against the returned app opens it.
    await goOnline(fixture);
    const gated = await agentCall(fixture);
    expect(gated.body.error?.code).toBe(-32003);
    expect(await approvalRows(fixture)).toHaveLength(1);
  ...
  it("13. §7 · an approved pass survives a -32000 for an offline app untouched: the row still reads `approved` and the later retry executes once", async () => {
    const fixture = await seedFixture();
    const pass = await seedApprovedPass(fixture);
    await goOffline(fixture);
  ...
  it("14. §7 · unavailability discovered BETWEEN check and claim yields -32000 with the pass unconsumed — twin: availability holding through the same window consumes it and executes", async () => {
    ...
    await goOnline(fixture);
    const executed = await agentCall(fixture);
    expect(executed.body.result).toBeDefined();
    expect((await approvalRow(fixture, pass)).status).toBe("used");
  }, CASE_BUDGET_MS);

  it("15. §7 · a dispatch failure AFTER a successful claim leaves the pass consumed (a call may already have reached the app, §15) and the retry gets a fresh -32003", async () => {
    const fixture = await seedFixture();
    const pass = await seedApprovedPass(fixture);
    fixture.fake.setBehavior(TOOL, { mode: "hang" });

    const inFlight = agentCall(fixture);
    expect(await waitFor(() => servedCalls(fixture) > 0)).toBe(true);
...
describe("§7 MRTR — one approval, many legs", () => {
  it("16. §7 · an approved call whose relayed result is input_required restores the row to `approved`, and the consumer receives that result verbatim", async () => {
    ...
    const leg = await agentCall(fixture);
  ...
  it("17. §7 · the follow-up leg carrying inputResponses/requestState rides the original approval and reaches the app (the counter advances per leg, no new -32003)", async () => {
    const fixture = await seedFixture();
    const pass = await seedApprovedPass(fixture);
    fixture.fake.setBehavior(TOOL, { mode: "input_required", result: INPUT_REQUIRED });
    await agentCall(fixture);

    fixture.fake.setBehavior(TOOL, { mode: "answer", result: ANSWER });
    const followUp = await agentCall(fixture, ARGS, {
  ...
  it("18. §7 · a complete result consumes the pass: the next identical call opens a fresh pending", async () => {
    ...
    await agentCall(fixture);
    const next = await agentCall(fixture);
...
describe("§7 the redaction union and the catalog", () => {
  it("20. §7 · the stored args_json masks the UNION over the live catalog: schema `writeOnly` paths from the DO's cached inputSchema plus config `redact` paths, while the app still receives the real values", async () => {
    const fixture = await seedFixture();
    const secretive = { ...ARGS, token: ARG_SECRET, profile: { email: CONFIG_SECRET } };

    const gated = await agentCall(fixture, secretive);
    const id = approvalIdOf(gated);
    await approvals().decide(fixture.ns.owner.userId, id, "approve");
    await agentCall(fixture, secretive);
    ...
    // …and the config half — `profile.email` is the app's own `redact` entry.
    ...
    // The APP is handed the real values: masking exists for persistence (§15).
  ...
  it("21. §7 · args_hash is post-redaction: two calls differing only in a redacted field share one approval row — twin: differing in a visible field, they do not (case 5)", async () => {
    ...
    const first = await agentCall(fixture, { ...ARGS, token: ARG_SECRET });
    const second = await agentCall(fixture, { ...ARGS, token: "FAKE0000-a-different-secret" });
  ...
  it("22. §7 · a tool absent from the cached catalog is refused -32001 — the same code as ungranted and unknown, with no pending row created; twin: the catalogued tool opens one (case 1)", async () => {
    ...
    const missing = await agentCall(fixture, ARGS, {}, PHANTOM_TOOL);
    ...
    expect((await agentCall(fixture)).body.error?.code).toBe(-32003);
    expect(await approvalRows(fixture)).toHaveLength(1);
  }, CASE_BUDGET_MS);

  // Why the fixture registers for real and takes the CATALOG cold rather than skipping the
  // socket: an app that has never held one has an empty `roles_json` (declared roles are
  ...
  it("23. §7 · availability outranks the catalog: an app that registered `reader` but served an EMPTY catalog and then went offline is refused -32000 — never case 22's -32001 — with no approval row created · then it reconnects serving the catalog and the IDENTICAL call opens a pending -32003 carrying an approvalId — exactly one row, and the app still executed nothing", async () => {
    ...
    await untilStatus(fixture.app.id, "online");
    const session = await seedOwnerSession(fixture.ns.owner);
    const listed = await rpc(fixture, session.token, APP_SLUG, {
      jsonrpc: "2.0",
      id: CONSUMER_ID,
      method: "tools/list",
      params: {},
    ...
    // The healing half: the app returns with its catalog and the IDENTICAL call — same
    // agent, same arguments, only the world between them changed — opens the pending the
    ...
    expect(servedCalls(fixture), "the app still executed nothing").toBe(0);
  }, CASE_BUDGET_MS);
});
