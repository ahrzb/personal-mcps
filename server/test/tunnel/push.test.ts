/**
 * tunnel/push.test.ts — §21's two new facts about the app DO: it holds a SECOND CLASS
 ...
 * selects the app socket by TAG, never by position — and this file is where that stops
 * being a claim: each case here accepts a subscriber socket FIRST, so an implementation
 * that still reads `getWebSockets()[0]` answers "offline" for an online app, forwards a
 ...
 * when the app says something changed: absent and `[]` compare equal, a re-warm that
 ...
 * `--no-isolate` every case mints its own namespace and app, so a stale socket from
...
// deps: harness/seed · harness/fake-app (connectFakeApp, openSubscriber, tick, waitFor) · harness/tunnel-do (backendCtx, connectionStub, stillOpen, untilStatus) · cloudflare:test (env, runDurableObjectAlarm, runInDurableObject) · src/tunnel (tunnelBackend, sever, status, wipe, close codes) · src/capabilities (BELL_*, subscriberTag) · src/audit (query) · src/limits (LISTEN_BELL_MIN_INTERVAL_MS) · src/registry (Registry)
...
/** One namespace with one tunneled app and two live tokens — the second exists only so
...
/** Two tunneled apps in ONE namespace — the cross-app burst §21.7 records as
...
async function warmed(
  ...
  const app = await connect(fixture, options);
  expect(await app.registered).toEqual({ ok: true });
  const warms = expectedWarms({ tools: [], ...options });
  expect(await waitFor(() => listCount(app) >= warms), "the registration never warmed").toBe(
    true,
  );
 ...
 */
function expectedWarms(options: Partial<FakeAppOptions>): number {
  const declared =
 ...
 * un-awaited notifications — the fake app installs its new catalog before it sends the
 ...
 */
async function changeTools(app: FakeApp, tools: Tool[]): Promise<void> {
  const listed = listCount(app);
  await app.notifyToolsListChanged(tools);
  expect(await waitFor(() => listCount(app) > listed), "the re-list never arrived").toBe(true);
  await settle();
}

/** The same, for the prompt catalog. */
async function changePrompts(app: FakeApp, prompts: CatalogEntry[]): Promise<void> {
  const listed = listCount(app);
  await app.notifyPromptsListChanged(prompts);
  expect(await waitFor(() => listCount(app) > listed), "the re-list never arrived").toBe(true);
  await settle();
}

/** …and for the resource catalogs, which one frame re-lists BOTH of (§20.5). */
async function changeResources(app: FakeApp, resources: CatalogEntry[]): Promise<void> {
  const listed = listCount(app);
  await app.notifyResourcesListChanged(resources);
  expect(await waitFor(() => listCount(app) >= listed + 2), "the re-list never arrived").toBe(
    true,
  );
  await settle();
}
...
async function socketCounts(fixture: Fixture): Promise<{ app: number; subscribers: number }> {
  return runInDurableObject(
    ...
    (_instance: AppConnection, state) => {
      ...
      return { app: tags.length - subscribers, subscribers };
    },
  );
}
...
async function untilSocketCounts(
  fixture: Fixture,
  expected: { app: number; subscribers: number },
): Promise<{ app: number; subscribers: number }> {
...
async function scheduledAlarm(fixture: Fixture): Promise<number | null> {
  return runInDurableObject(connectionStub(fixture.appId), (_i: AppConnection, state) =>
...
describe("§21.2 subscriber sockets are a class of their own", () => {
  it('§21.2 · a subscriber socket is accepted through the DO\'s fetch door tagged "sub:<session-id>" — the one structural pin; everything else it carries is observed through routing below', async () => {
    ...
    const tags = await runInDurableObject(
      connectionStub(fixture.appId),
      (_instance: AppConnection, state) => state.getWebSockets().map((ws) => state.getTags(ws)),
    );
  ...
  it("§21.2 · the socket reader selects by CLASS, never by position — with a subscriber socket accepted FIRST and the app socket second: status reads online, a tools/call reaches the app, and with only subscriber sockets held status reads offline", async () => {
    const fixture = await seedFixture();
    const stream = await subscriber(fixture);
    const app = await warmed(fixture, { tools: [TOOL] });
    ...
    // The call reached the APP, not the stream that was sitting in front of it: the
    // only thing this stream ever heard is the doorbell that first registration rang.
    expect(app.callCount(TOOL.name)).toBe(1);
  ...
  it("§21.2/§6 · the DO holds at most one APP socket beside its subscriber sockets — §6's at-most-one-connection invariant counts app sockets only", async () => {
    const fixture = await seedFixture();
    ...
    expect(await untilSocketCounts(fixture, { app: 1, subscribers: 2 })).toEqual({
      app: 1,
      subscribers: 2,
    });
  ...
  it("§21.2/§6 · a newer app connection evicts the old APP socket with hub/replaced and close 4000, leaving every subscriber socket open", async () => {
    const fixture = await seedFixture();
    const stream = await subscriber(fixture);
    const first = await warmed(fixture);
  ...
  it("§21.2/§15 · a targeted token revoke severs the APP socket with 4001 even with a subscriber socket sitting first in the untagged enumeration · a rotation revoke naming another token id leaves app and subscriber sockets alike open (the twin)", async () => {
    const fixture = await seedFixture();
    const stream = await subscriber(fixture);
  ...
  it("§21.2/§6 · app delete closes every subscriber socket alongside the app socket and the wiped state · archive severs the app socket alone (4002) and leaves every subscriber socket open — the archived case reaches streams through the re-auth tick (the twin)", async () => {
    ...
    await sever(deleted.appId, CLOSE_REVOKED);
    await wipe(deleted.appId);
    expect((await going.closed).code).toBe(CLOSE_REVOKED);
    expect((await doomed.closed).code).toBe(CLOSE_REVOKED);
    expect(await connectionStub(deleted.appId).listTools()).toEqual([]);
    ...
    await sever(archived.appId, CLOSE_ARCHIVED);
  ...
  it("§21.2/§6 · the registration-deadline alarm closes the unregistered APP socket with 4004 and leaves every subscriber socket open · accepting a subscriber socket neither arms nor re-arms that deadline (the twin)", async () => {
    const fixture = await seedFixture();
    const stream = await subscriber(fixture);
    ...
    // The twin, on a DO no app has ever reached: a subscriber accept arms nothing.
    const quiet = await seedFixture();
    const alone = await subscriber(quiet);
    expect(await scheduledAlarm(quiet)).toBe(null);
    expect(await runDurableObjectAlarm(connectionStub(quiet.appId))).toBe(false);
...
describe("§21.3 the bell rings at the write", () => {
  it("§21.3 · a re-warm whose catalog changed rings that family's bell on every subscriber socket the DO holds · a re-warm producing an equal catalog rings nothing however loudly the app sent list_changed — coalescer drained before concluding (the twin)", async () => {
    const fixture = await seedFixture();
    const app = await warmed(fixture);
    ...
    // Twice, with the same catalog both times: the app is as loud as it can be.
  ...
  it("§21.3/§21.5 · a first registration warming a NON-EMPTY catalog rings — absent and non-empty differ, which is what makes the never-connected capability honest · the same first registration warming [] rings nothing, drained (the twin — absent ≡ [])", async () => {
    const fixture = await seedFixture();
    // The stream opens BEFORE the app has ever connected — the never-connected case.
    const stream = await subscriber(fixture);
  ...
  it("§21.3/§20.5 · a successful registration that undeclares a NON-EMPTY family clears it and rings its bell · undeclaring an already-empty family rings nothing, drained (the twin)", async () => {
    ...
    await served.close();
    await untilStatus(fixture.appId, "offline");
    // The same app, now declaring tools alone: §20.5's clear IS a catalog change.
    await warmed(fixture, { capabilities: ["tools"] });
    await drain(fixture);
    expect(stream.count(BELL_PROMPTS)).toBe(1);
    expect(stream.count(BELL_TOOLS)).toBe(0);
    expect(await connectionStub(fixture.appId).listCatalog("prompts")).toEqual([]);

    const quiet = await seedFixture();
    const empty = await warmed(quiet, { prompts: [] });
    const deaf = await subscriber(quiet);
    await empty.close();
    await untilStatus(quiet.appId, "offline");
  ...
  it("§21.3/§20.5 · a warm that fails or times out rings nothing, drained — a failure is not an undeclare", async () => {
    ...
    // The catalog the app WOULD serve is empty now — so a warm that landed would clear
    // a non-empty family and ring. Neither of these lands.
    app.setListBehavior({ mode: "error", error: { code: -32603, message: "not now" } }, "tools");
    await app.notifyToolsListChanged([]);
    expect(await waitFor(() => listCount(app) >= 2)).toBe(true);
    app.setListBehavior({ mode: "hang" }, "tools");
    await app.notifyToolsListChanged([]);
    expect(await waitFor(() => listCount(app) >= 3)).toBe(true);

    await drain(fixture);
    expect(stream.frames).toEqual([]);
    // …and the cache still holds what neither failure was allowed to empty.
    expect(await connectionStub(fixture.appId).listTools()).toEqual([TOOL]);
  });
  ...
  it("§21.3/§6 · a warm that changes both resource catalogs rings notifications/resources/list_changed ONCE · one that changes only the templates list still rings it (the twin — MCP defines no templates frame)", async () => {
    const fixture = await seedFixture();
    const app = await warmed(fixture, { resources: [], resourceTemplates: [] });
    const stream = await subscriber(fixture);

    app.setCatalog("resourceTemplates", [TEMPLATE]);
  ...
  it("§21.3 · the frame on a subscriber socket is the bare method-only notification — no catalog, no entry names, no app id", async () => {
    const fixture = await seedFixture();
    const app = await warmed(fixture);
    const stream = await subscriber(fixture);
    ...
    expect(wire).not.toContain(fixture.appId);
  });
  ...
  it("§21.3/§21.2 · the DO rings every subscriber socket it holds, resources bell included — the shape filter is the Worker's, and the DO knows no shapes", async () => {
    const fixture = await seedFixture();
    const app = await warmed(fixture, { resources: [] });
    const listeners = [
      await subscriber(fixture),
      await subscriber(fixture),
      await subscriber(fixture),
...
describe("§21.3 the leading-edge floor", () => {
  ...
  it("§21.3 · N changes inside one interval deliver the leading frame now and ONE trailing frame when the DO's alarm fires — at most two frames, and the final state always rings", async () => {
    ...
    // Each change must LAND before the next is announced: the fake app installs its
    ...
    const startedAt = Date.now();
    await changeTools(app, [toolNamed("one")]);
    await changeTools(app, [toolNamed("two")]);
    await changeTools(app, [toolNamed("three")]);
    expect(await waitFor(() => stream.count(BELL_TOOLS) > 0)).toBe(true);
    expect(
      Date.now() - startedAt,
    ...
    expect(await connectionStub(fixture.appId).listTools()).toEqual([toolNamed("three")]);
  });
  ...
  it("§21.3 · the floor is per family — a tools burst inside one interval never suppresses that interval's prompts or resources bell", async () => {
    const fixture = await seedFixture();
    const app = await warmed(fixture, { prompts: [], resources: [] });
    const stream = await subscriber(fixture);

    const startedAt = Date.now();
    await changeTools(app, [toolNamed("one")]);
    await changeTools(app, [toolNamed("two")]);
    await changePrompts(app, [PROMPT]);
    await changeResources(app, [RESOURCE]);
    expect(
      Date.now() - startedAt,
      "the burst outran the floor's window, so this is no longer one interval",
    ).toBeLessThan(LISTEN_BELL_MIN_INTERVAL_MS);
  ...
  it("§21.3/§21.7 · the floor is per app DO — two granted apps changing the same family inside one interval deliver two frames, the cross-app burst §21.7 records as uncoalesced", async () => {
    const [first, second] = await seedPair();
    const alpha = await warmed(first);
    const beta = await warmed(second);
    // One stream, two DOs: the Worker opens a socket per app under one session id.
  ...
  it("§21.3/§6 · the two alarm purposes are multiplexed, never clobbered — an app (re)connect while a ring is pending does not cancel the ring, and an armed registration deadline still closes an unregistered socket while a ring is pending", async () => {
    const fixture = await seedFixture();
    const app = await warmed(fixture);
    const stream = await subscriber(fixture);
    ...
    // A newcomer arrives mid-burst: it evicts the app socket and arms the deadline.
    const newcomer = await connect(fixture, { skipRegister: true });
    expect((await app.closed).code).toBe(CLOSE_REPLACED);
