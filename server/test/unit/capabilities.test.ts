...
// is a function of the service's KIND — "tunnel" | "proxy" | "builtin" — and of the
// stored capability set only in WHICH families appear. Proxied services keep every
...
// capability set for a registered service feeds `stored` (tunnel/push.test.ts). This
...
import type { ServiceCapability } from "../../src/registry";
...
describe("§21.5 · capabilityShape — the kind-aware capability picture", () => {
  it("§21.5 · shape([\"tools\",\"resources\"], \"tunnel\") deep-equals {tools: {listChanged: true}, resources: {listChanged: true, subscribe: true}} — no subscribe key on a non-resources family, no family the stored set lacks", () => {
    const stored: ServiceCapability[] = ["tools", "resources"];
    ...
    // canonical SERVICE_CAPABILITIES order (never the stored set's own, which is
    ...
    // render tools first.
    expect(Object.keys(capabilityShape(["resources", "tools"] as ServiceCapability[], "tunnel"))).toEqual([
      "tools",
      "resources",
    ]);
  ...
  it("§21.5 · shape(DEFAULT_SERVICE_CAPABILITIES, \"tunnel\") deep-equals {tools: {listChanged: true}} — the never-connected service's answer, and the unresolvable slug's identical one (§20.2's anti-enumeration)", () => {
    expect(capabilityShape(DEFAULT_SERVICE_CAPABILITIES, "tunnel")).toEqual({
      tools: { listChanged: true },
    });
  });
  ...
  it("§21.5 · kind \"builtin\" forces every push flag false on the same default set — {tools: {listChanged: false}} — the kind-fallback an is-not-proxy implementation gets wrong", () => {
    expect(capabilityShape(DEFAULT_SERVICE_CAPABILITIES, "builtin")).toEqual({
      tools: { listChanged: false },
    });
  });
  ...
  it("§21.3/§21.5 · a stored completions shapes as the empty object on every kind — only the three bell-ringing families carry a listChanged flag, because no completions bell exists", () => {
    const stored: ServiceCapability[] = ["completions"];
  ...
  it("§21.5/§20.2 · the aggregated constant is tools and prompts both {listChanged: true}, no resources, no completions, no subscribe — pinned here so the fixture regeneration is not the only guard", () => {
    ...
    // The same SERVICE_CAPABILITIES order the scoped pictures render in, so the
