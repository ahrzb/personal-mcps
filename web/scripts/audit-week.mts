...
//
// The type import is RELATIVE, not \`@/\`: \`server/test/unit/audit-derive.test.ts\` reads this week
// to assert that the merged list never steps forward in time, and that test compiles under the
// repo's ROOT tsconfig, which has no \`@/\` path mapping.
